...
####
history | tac | bat -ppfljava | cut -f 5- -d " " | fzf --border none --wrap --wrap-sign="" --bind 'focus:transform-header:(echo -e ""{1}"")' --header-first --tmux '100%,98%' --delimiter="_" --with-nth "{2..}" --no-preview --accept-nth "{2..}" > ~/logs/h/${date}.log; 
####
####
