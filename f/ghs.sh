...
ghs_folder="${HOME}/logs/zz/tmp/ghs"; 
...
for i in ${oo[*]}; do 
printf %b "$i" > ${ghs_folder}/$(printf %b "${i/\ */}"|sed -e 's/\//@/';); done; 
ghss=($(command ls -tr "${ghs_folder}" | fzf -i -m --preview "clear; cat $ghs_folder/{}|sed -e 's/\ /\n--\n/'|bat -ppflzig" --preview-window '9,top,wrap' --bind 'q:abort' \
