...
#ipt=$(mktemp; ); 
mkdir -p $HOME/logs/zz/iplocal/ 2>/dev/null; 
rm $HOME/logs/zz/iplocal/* 2>/dev/null;  
c5='    \e[96m--\e[0m'; 
c6='\e[96m--\e[0m'; 
wlan="$(cat $HOME/logs/zz/wlan.log)"; 
iploc="$(cat $HOME/logs/zz/wlan.log)"; 
# printf %b "\n\n\n\n\e[4A\n\e[96m --\n\e[0m "; 
...
rm $HOME/logs/zz/iplocal/$iploc 2>/dev/null; 
iplocal=($(command ls $HOME/logs/zz/iplocal; )); 
# iplocall=-($iploc);
# command ls "$HOME/logs/zz/iplocal"; echo; 

# for ip in ${iplocal[*]}; do (nmap --open $ipp -p 22,8022 --system-dns 2>/dev/null|grep -e "open"|cut -f1 -d"/"|tr -s "\n" " ") > $HOME/logs/zz/iplocal/$ipp; done; 
...
# ops=($(command ls -1 $HOME/logs/zz/iplocal)); 
# local desc=()
