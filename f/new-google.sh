...
# 
cat ${pos}/google_${sub}_${time}_.xml | sed -e "s/<*>/&\n/g" | sed -n "/<pubDate>/,/<\/description>/p"|sed -e "s/\&lt\;/</g" -e "s/\&gt\;/>/g" -re 's/(nbsp|&amp)\;//g' -e "s/<description>/<pre>/g" -e "s/<\/description>//g" -e "s/<pubDate>/\ -\ /g" -e "s/\/pubDate>/\/pre>_/g" -e "s/<\/a>/&\ -\ /g" -e "1,2d"|tr -d "\n"|sed -e "s/_/\n\n/g" >> ${pos}/google_${sub}_${time}_.html; 
##
